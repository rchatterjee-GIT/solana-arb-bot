...
async function backgroundWalletClean() {
      ...
      for (const d of details) {
        ...
        try {
          ...
          if (result.code === '0') {
            ...
            await sendAlert('🤖 [BOT] Cleaned: ' + d.ccy + ' on OKX\n$' + usdEst.toFixed(2) + ' → USDT');
          } else {
    ...
    for (const c of coins) {
      ...
      try {
        ...
        if (sold) {
          ...
          await sendAlert('🤖 [BOT] Cleaned: ' + c.coin + ' on Bybit\n$' + usdEst.toFixed(2) + ' → USDT');
        } else {
  ...
  try {
    ...
    if (usdtAmt >= 1) {
      ...
      await sendAlert(`🧹 <b>Cleaned: USDT on Solana\n$${usdtAmt.toFixed(2)} → $${usdcOut.toFixed(2)} USDC`);
      anyFound = true;
    }
  ...
  try {
    ...
    for (const pair of allPairs) {
      ...
      try {
        ...
        await sendAlert('🤖 [BOT] Cleaned: ' + pair.okxCcy + ' on Solana\n$' + usdEst.toFixed(2) + ' → $' + usdcOut.toFixed(2) + ' USDC');
  ...
  try {
    ...
    const pairResults = await Promise.allSettled(
      activePairs.map(async (pair) => {
        ...
        const timestamp = new Date().toLocaleTimeString();
        console.log(`[${timestamp}] ${pair.name.padEnd(11)}${!okxViable?'[Os]':''}${!bybitViable&&pair.bybitCcy?'[Bs]':''}${!dexEnabled?'[Ds]':''} OKX:${okx.bid}/${okx.ask} →OKX:${spreadOKX.toFixed(2)}% →By:${bybit?spreadBybit.toFixed(2):'--'}% →DEX:${spreadDex.toFixed(2)}%(≥${dexThresh.toFixed(2)}%)`);
