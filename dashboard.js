...
async function fetchOKX() {
  try {
    ...
    var r = await fetch('https://www.okx.com/api/v5/account/balance', {
      headers: {
        'OK-ACCESS-KEY': process.env.OKX_API_KEY,
        'OK-ACCESS-SIGN': okxSign(ts,'GET','/api/v5/account/balance'),
        'OK-ACCESS-TIMESTAMP': ts,
        'OK-ACCESS-PASSPHRASE': process.env.OKX_PASSPHRASE
      }
    });
    var j = await r.json();
    return parseFloat((j.data||[{}])[0].details.find(function(d){return d.ccy==='USDT';}).availBal||'0');
  } catch(e) { return null; }
}

async function fetchBybit() {
  try {
    var ts = ''+Date.now(), rw = '5000', qs = 'accountType=UNIFIED&coin=USDT';
    var sig = crypto.createHmac('sha256', process.env.BYBIT_API_SECRET).update(ts+process.env.BYBIT_API_KEY+rw+qs).digest('hex');
    var r = await fetch('https://api.bybit.com/v5/account/wallet-balance?'+qs, {
      headers: {'X-BAPI-API-KEY':process.env.BYBIT_API_KEY,'X-BAPI-TIMESTAMP':ts,'X-BAPI-SIGN':sig,'X-BAPI-RECV-WINDOW':rw}
    });
    var j = await r.json();
    var coin = (j.result&&j.result.list&&j.result.list[0]&&j.result.list[0].coin||[]).find(function(c){return c.coin==='USDT';});
    return Math.max(parseFloat((coin&&coin.equity)||'0'), parseFloat((coin&&coin.walletBalance)||'0')*0.95);
  } catch(e) { return null; }
}

async function fetchSolana() {
  try {
    var {Connection,Keypair,PublicKey} = require('@solana/web3.js');
    var {getAssociatedTokenAddress,getAccount} = require('@solana/spl-token');
    var conn = new Connection(process.env.RPC_URL,'confirmed');
    var wallet = Keypair.fromSecretKey(Uint8Array.from(JSON.parse(process.env.PRIVATE_KEY)));
    var USDC = new PublicKey('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v');
    var ata = await getAssociatedTokenAddress(USDC, wallet.publicKey);
    var acc = await getAccount(conn, ata);
...
function getData() {
  ...
  var trades  = readJSON(TRADES_FILE) || [];
  var fires   = readJSON(FIRES_FILE) || [];
  var log     = readJSON(LOG_FILE) || {};
  var live    = readJSON(LIVE_FILE) || null;
  var status  = readJSON(STATUS_FILE) || null;
  var config  = readJSON(CONFIG_FILE) || {};
  var trLog   = readJSON(TRADELOG) || [];
  var simTr   = readJSON(SIMTRADES) || [];

  var recentTrades = trades.slice(-10).reverse();
  var recentFires  = fires.slice(-20).reverse();
  ...
  var balHistory = [];
  Object.entries(log).slice(-7).forEach(function(e) {
    (e[1].reports||[]).forEach(function(r) {
      balHistory.push({time: e[0]+' '+r.time, total: r.total, okx: r.okxUsdt, solana: r.solanaUsdc, bybit: r.bybitUsdt});
    });
  });

  var now = Date.now(), oneWeek = 7*24*60*60*1000;
  var week = trades.filter(function(t){return new Date(t.date).getTime()>now-oneWeek;});
  var allWins = trades.filter(function(t){return t.profit>0;}).length;
  var weekWins = week.filter(function(t){return t.profit>0;}).length;
  var weekPnl = week.reduce(function(a,t){return a+(t.profit||0);},0);
  var startCapital = state.startCapital||0;
  var tradingProfit = state.totalProfit||0;
  var injRatio = startCapital ? (tradingProfit/startCapital*100) : 0;
  var latest = balHistory[balHistory.length-1]||{};
  var allWinPct = trades.length ? Math.round(allWins/trades.length*100) : 0;
  var weekWinPct = week.length ? Math.round(weekWins/week.length*100) : 0;

  return {
    state, recentTrades, recentFires, pairStats, balHistory, live, status, config,
    tradeLog: trLog, simTrades: simTr,
    allWinPct, weekWinPct, weekPnl, latest,
    allTrades: trades.length, weekTrades: week.length,
    allWins, weekWins, startCapital, tradingProfit, injRatio,
...
function H() {
  return '<!DOCTYPE html><html lang="en"><head>' +
    ...
    '.sec{color:#a78bfa;font-size:.68rem;text-transform:uppercase;letter-spacing:.08em;' +
         'margin:12px 0 6px;border-bottom:1px solid #1e1e30;padding-bottom:3px;display:flex;justify-content:space-between}' +
    ...
    '<div style="background:#0f0f1a;border:1px solid #1e1e30;border-radius:6px;padding:8px 12px;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center;font-size:.72rem">' +
      '<span><span style="color:#555">Version: </span><span id="dv" style="color:#a78bfa;font-weight:bold">-</span>' +
      ...
      ' <span style="color:#555;margin-left:12px">P&L: </span><span id="dp" style="color:#888">-</span></span>' +
      '<span id="dbk" style="display:none"><button class="btn" style="border-color:#666;color:#666;font-size:.65rem" onclick="doRollback()">Rollback</button></span>' +
    '</div>' +
    ...
    '<div class="g2">' +
      '<div class="card"><h2 style="color:#a78bfa;font-size:.72rem;text-transform:uppercase;margin-bottom:8px">System Status</h2>' +
        '<table>' +
        ...
        '</table>' +
      '</div>' +
      '<div class="card"><h2 style="color:#a78bfa;font-size:.72rem;text-transform:uppercase;margin-bottom:8px">Pair Viability</h2>' +
        ...
        '<div style="margin-bottom:6px"><span class="dim">DEX: </span><span style="color:#a78bfa;font-size:.72rem">SOL JTO WIF RAY PYTH W RENDER PENGU</span></div>' +
    ...
    '<div class="card g1"><table><thead><tr><th>Pair</th><th>OKX</th><th>Bybit</th><th style="text-align:right">OKX%</th><th style="text-align:right">Bybit%</th><th style="text-align:right">DEX%</th><th>Status</th></tr></thead><tbody id="lt"></tbody></table></div>' +

    '<div class="sec">Solana Wallet</div>' +
    '<div class="card g1"><div id="tk" style="color:#444;font-size:.72rem">Loading...</div></div>' +

    '<div class="g2">' +
      '<div class="card"><h2 style="color:#a78bfa;font-size:.72rem;text-transform:uppercase;margin-bottom:8px">Capital 7d</h2><canvas id="ch" height="100"></canvas></div>' +
      '<div class="card"><h2 style="color:#a78bfa;font-size:.72rem;text-transform:uppercase;margin-bottom:8px">Recent Fires</h2>' +
        '<table><thead><tr><th>Time</th><th>Pair</th><th>Dir</th><th>Result</th></tr></thead><tbody id="ft"></tbody></table>' +
      '</div>' +
    '</div>' +

    '<div class="g2">' +
      '<div class="card"><h2 style="color:#a78bfa;font-size:.72rem;text-transform:uppercase;margin-bottom:8px">Recent Trades</h2>' +
        '<table><thead><tr><th>Time</th><th>Pair</th><th>Dir</th><th>Spread</th><th>P&L</th></tr></thead><tbody id="tt"></tbody></table>' +
      '</div>' +
      '<div class="card"><h2 style="color:#a78bfa;font-size:.72rem;text-transform:uppercase;margin-bottom:8px">Pair Stats</h2>' +
        '<table><thead><tr><th>Pair</th><th>Fires</th><th>Win%</th><th>P&L</th></tr></thead><tbody id="pt"></tbody></table>' +
      '</div>' +
    '</div>' +

    '<div id="rm" class="modal"><div class="mbox">' +
      ...
      '<div style="display:flex;gap:8px"><button onclick="execRebalance()" id="re" class="btn" style="border-color:#22c55e;color:#22c55e;flex:1">Execute</button><button onclick="closeR()" class="btn" style="flex:1">Cancel</button></div>' +
    '</div></div>' +
    ...
    'async function doBalances(){' +
      ...
      'try{' +
        'var r=await fetch("/api/live-balances");' +
        'var d=await r.json();' +
        'liveBal=d;' +
        'renderBal(d);' +
    ...
    'async function doRebalance(){' +
      ...
      'try{' +
        ...
        'else{h+="<b style=color:#eab308>Recommended:</b><br>";' +
          'if(d.toOKX>5)h+="Move $"+d.toOKX+" to OKX<br>";' +
          'if(d.toBybit>5)h+="Move $"+d.toBybit+" to Bybit<br>";' +
    ...
    'async function execRebalance(){' +
      ...
      'try{' +
        'await fetch("/api/rebalance-execute",{method:"POST"});' +
        'document.getElementById("rc").innerHTML="<span class=green>Rebalance sent to bot. Check Telegram.</span>";' +
      '}catch(e){document.getElementById("rc").innerHTML="<span class=red>"+e.message+"</span>";}' +
    '}' +
    ...
    'async function doRollback(){' +
      ...
      'try{var r=await fetch("/api/rollback",{method:"POST"});var d=await r.json();if(d.ok)alert("Rollback complete. Restored: "+d.restored.join(", "));else alert("Failed: "+d.error);}catch(e){alert(e.message);}' +
    '}' +
    ...
    'function renderToks(data){' +
      ...
      'var SYMS={"So11111111111111111111111111111111111111112":"SOL","EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v":"USDC","Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB":"USDT","jtojtomepa8beP8AuQc6eXt5FriJwfFMwQx2v2f9mCL":"JTO","EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm":"WIF","DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263":"BONK","JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN":"JUP","4k3Dyjzvzp8eMZWUXbBCjEvwSkkk59S5iCNLY3QrkX6R":"RAY","85VBFQZC9TZkfaptBWjvUw7YbZjy52A6mjtPGjstQAmQ":"W","2qEHjDLDLbuBgRYvsxhc5D6uDWAivNFZGan56P1tpump":"PNUT","CzLSujWBLFsSjncfkh59rUFqvafWcY5tzedWJSuypump":"GOAT","2zMMhcVQEXDtdE6vsFS7S7D5oUodfJHE8vd1gnBouauv":"PENGU"};' +
      'var rows=[];' +
      'Object.keys(data.toks||{}).forEach(function(mint){' +
        'var sym=SYMS[mint]||mint.slice(0,6);' +
        'var bal=data.toks[mint];' +
        'var price=parseFloat((data.prices&&data.prices[mint]&&data.prices[mint].price)||0);' +
        'var usd=bal*price;' +
        'rows.push({sym:sym,bal:bal,usd:usd,price:price});' +
      '});' +
      'rows.sort(function(a,b){return b.usd-a.usd;});' +
      'el.innerHTML=rows.map(function(r){' +
        'var dust=r.usd<0.01;' +
        'return "<span style=\"display:inline-block;min-width:120px;margin:3px 8px 3px 0;vertical-align:top;"+(dust?"opacity:.3":"")+"\"><b class=purple>"+r.sym+"</b><br>"+' +
          '(r.bal<1?r.bal.toFixed(4):r.bal.toFixed(2))+" <span class=green>"+(r.usd>0?"$"+r.usd.toFixed(2):"-")+"</span></span>";' +
      '}).join("");' +
    '}' +
    'function render(d){' +
      'var st=d.status;' +
      ...
      'if(active>0){pill.className="pill pa";pill.textContent=active+" active";}' +
      'else{pill.className="pill pq";pill.textContent="watching";}' +
      'var latest=d.latest||{};' +
      ...
      'var wins=d.state&&d.state.consecutiveWins||0;' +
      'var clean=d.state&&d.state.consecutiveClean||0;' +
      'var tgt=10;' +
      'var wbar="";for(var i=0;i<tgt;i++)wbar+="<div class=\"wdot "+(i<wins?"wf":"we")+"\"></div>";' +
      'document.getElementById("wb").innerHTML=wbar;' +
      'document.getElementById("wt").textContent=wins+"/"+tgt+" consecutive";' +
      'var cbar="";for(var i=0;i<tgt;i++)cbar+="<div class=\"wdot\" style=\"background:"+(i<clean?"#22c55e":"#1e1e30")+"\"></div>";' +
      'document.getElementById("cb").innerHTML=cbar;' +
      'document.getElementById("ct").textContent=clean+"/"+tgt+" clean";' +
      ...
      'document.getElementById("wd").textContent=d.allWins+"W / "+(d.allTrades-d.allWins)+"L - "+d.allTrades+" trades";' +
      ...
      'var krakenOn=d.config&&d.config.KRAKEN_ENABLED;' +
      'var krakenSim=d.config&&d.config.KRAKEN_SYNTHETIC;' +
      ...
      'var kvr=document.getElementById("kvr");' +
      'if(krakenOn){kvr.style.display="";document.getElementById("kv").innerHTML="<span class=\"badge bg\">SOL</span><span class=\"badge bg\">PENGU</span>";}' +
      'else{kvr.style.display="none";}' +
      'var allOKX=["SOL","JTO","WIF","W","MEW","PNUT","GOAT","PENGU","PYTH","RAY"];' +
      'var allBybit=["SOL","JTO","WIF","W","RENDER","PNUT","PENGU"];' +
      'var skipOKX=st&&st.skipOKX||[];' +
      'var skipBybit=st&&st.skipBybit||[];' +
      ...
      'if(pend.length>0){' +
        ...
        'document.getElementById("ifl").innerHTML=pend.map(function(t){' +
          'var el=Math.round((Date.now()-t.startTime)/60000);' +
          'var pct=Math.min(100,Math.round(el/120*100));' +
          'return "<div style=\"display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #0f0f1a\"><span><b class=purple>"+t.symbol+"</b></span><span style=\"color:#eab308\">"+el+"min ("+pct+"%)</span></div>";' +
        '}).join("");' +
      ...
      'if(d.live&&d.live.pairs&&d.live.pairs.length>0){' +
        'lb.innerHTML=d.live.pairs.map(function(p){' +
          'var best=Math.max(p.spreadOKX,p.spreadBybit||0,p.spreadDex);' +
          'var thr=1.0;' +
          'var oc="<span style=color:"+sc(p.spreadOKX,thr)+">"+(p.spreadOKX>0?"+":"")+p.spreadOKX.toFixed(2)+"%</span>";' +
          'var bc=p.spreadBybit!=null?"<span style=color:"+sc(p.spreadBybit,thr)+">"+(p.spreadBybit>0?"+":"")+p.spreadBybit.toFixed(2)+"%</span>":"<span class=dim>--</span>";' +
          'var dc="<span style=color:"+sc(p.spreadDex,thr)+">"+(p.spreadDex>0?"+":"")+p.spreadDex.toFixed(2)+"%</span>";' +
          'var st2=best/thr>=1?"<span class=\"badge by\">FIRE</span>":"";' +
          'return "<tr><td><b>"+p.name.replace("/USDT","")+"</b></td><td class=dim style=font-size:.65rem>"+(p.okxBid?"$"+p.okxBid:"--")+"</td><td class=dim style=font-size:.65rem>"+(p.bybitBid?"$"+p.bybitBid:"--")+"</td><td style=text-align:right>"+oc+"</td><td style=text-align:right>"+bc+"</td><td style=text-align:right>"+dc+"</td><td>"+st2+"</td></tr>";' +
        '}).join("");' +
      ...
      'document.getElementById("ft").innerHTML=d.recentFires.map(function(f){' +
        ...
        'return "<tr><td class=dim style=white-space:nowrap>"+time+"</td><td>"+(f.pair||"").replace("/USDT","")+"</td><td><span class=\"badge bn\" style=font-size:.6rem>"+(f.direction||"").replace("BUY_","")+"</span></td><td>"+oc+"</td></tr>";' +
      '}).join("");' +
      'document.getElementById("tt").innerHTML=d.recentTrades.map(function(t){' +
        ...
        'return "<tr><td class=dim style=white-space:nowrap>"+date+"</td><td>"+(t.pair||"").replace("/USDT","")+"</td><td><span class=\"badge bn\" style=font-size:.6rem>"+(t.direction||"").replace("BUY_","")+"</span></td><td>"+(t.spreadPct||0).toFixed(2)+"%</td><td class="+(profit>=0?"green":"red")+">"+(profit>=0?"+":"")+"$"+profit.toFixed(2)+"</td></tr>";' +
      '}).join("");' +
      'document.getElementById("pt").innerHTML=Object.entries(d.pairStats).sort(function(a,b){return b[1].fires-a[1].fires;}).map(function(e){' +
        'var p=e[0],s=e[1];var wr=s.fires?Math.round(s.wins/s.fires*100):0;' +
    ...
    'scr.onload=function(){refresh();setInterval(refresh,3000);setInterval(loadStatus,30000);};' +
    'document.head.appendChild(scr);' +
    '</script></body></html>';
}

async function sendTelegramLocal(text) {
  try {
    await fetch('https://api.telegram.org/bot'+process.env.TELEGRAM_TOKEN+'/sendMessage', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({chat_id: process.env.TELEGRAM_CHAT_ID, text: text, parse_mode:'HTML'})
